...
fn is_number_invalid(value: i64, arr: *[32]u8) !bool {
    @memset(arr[0..], 0);
    var slice = try std.fmt.bufPrint(arr[0..], "{}", .{value});

    for (1..slice.len) |part_size| {
        if (slice.len % part_size != 0) {
            continue;
        }

        const a = slice[0..part_size];
        var i = part_size;

        while (i < slice.len) : (i += part_size) {
            const b = slice[i .. i + part_size];
...
fn iter_range(range: Range, logger: *std.io.Writer) !usize {
    ...
    while (value <= end) {
        const valid = try is_number_invalid(value, &arr);
